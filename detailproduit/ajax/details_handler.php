...
try {
    ...
    /**
     * Sauvegarder les détails pour une ligne de commande - Version corrigée
     */
    elseif ($action == 'save_details') {
        ...
        debug_log("Commandedet ID: " . $commandedet_id);
        debug_log("Details JSON length: " . strlen($details_json ?? ''));
        debug_log("Details JSON preview: " . substr($details_json ?? '', 0, 200) . "...");
        
        if (!$commandedet_id || !$details_json) {
            debug_log("ERREUR: Paramètres manquants");
            http_response_code(400);
            echo json_encode(array(
                'success' => false, 
                'error' => 'Missing required parameters: commandedet_id and details_json',
                'debug' => $debug_mode ? array(
                    'commandedet_id' => $commandedet_id,
                    'details_json_length' => strlen($details_json ?? ''),
                    'post_data' => $_POST
                ) : null
            ));
            exit;
        ...
        // Nettoyer et valider le JSON avant décodage
        $details_json_clean = trim($details_json);
        
        // Vérifications supplémentaires du JSON
        if (empty($details_json_clean)) {
            ...
            echo json_encode(array(
                ...
                'debug' => $debug_mode ? array('raw_data' => $details_json) : null
            ));
            exit;
        }
        
        if (!str_starts_with($details_json_clean, '[') || !str_ends_with($details_json_clean, ']')) {
            debug_log("ERREUR: JSON malformé - ne commence/finit pas par [ ]");
            http_response_code(400);
            echo json_encode(array(
                'success' => false, 
                'error' => 'Invalid JSON structure - expected array',
                'debug' => $debug_mode ? array(
                    'starts_with_bracket' => str_starts_with($details_json_clean, '['),
                    'ends_with_bracket' => str_ends_with($details_json_clean, ']'),
                    'first_chars' => substr($details_json_clean, 0, 10),
                    'last_chars' => substr($details_json_clean, -10)
                ) : null
            ));
            exit;
        }

        // Décoder le JSON des détails avec gestion d'erreur améliorée
        $details_array = json_decode($details_json_clean, true);
        $json_error = json_last_error();
        
        if ($json_error !== JSON_ERROR_NONE) {
            $json_error_msg = json_last_error_msg();
            debug_log("ERREUR: JSON invalide - Code: " . $json_error . ", Message: " . $json_error_msg);
            
            // Messages d'erreur JSON plus explicites
            $json_errors = array(
            ...
            $detailed_error = isset($json_errors[$json_error]) ? $json_errors[$json_error] : $json_error_msg;
            
            http_response_code(400);
            echo json_encode(array(
                'success' => false, 
                'error' => 'Invalid JSON format: ' . $detailed_error,
                'debug' => $debug_mode ? array(
                    'json_error_code' => $json_error,
                    'json_error_msg' => $json_error_msg,
                    'json_sample' => substr($details_json_clean, 0, 500),
                    'json_length' => strlen($details_json_clean)
                ) : null
            ));
            exit;
        }
        ...
        if (!is_array($details_array)) {
            ...
            echo json_encode(array(
                ...
                'debug' => $debug_mode ? array(
                    'decoded_type' => gettype($details_array),
                    'decoded_value' => $details_array
                ) : null
            ));
            exit;
        }

        debug_log("JSON décodé avec succès: " . count($details_array) . " éléments");

        // Valider et nettoyer les détails
        $validated_details = array();
        foreach ($details_array as $index => $detail) {
            if (!is_array($detail)) {
                debug_log("ATTENTION: Élément $index n'est pas un tableau");
                continue;
            }
            
            if (!isset($detail['pieces']) || !is_numeric($detail['pieces']) || $detail['pieces'] <= 0) {
                debug_log("ATTENTION: Élément $index - pieces invalide: " . ($detail['pieces'] ?? 'NULL'));
                continue; // Ignorer les lignes sans pièces valides
            }
            ...
            if (isset($detail['description']) && is_string($detail['description'])) {
                ...
                $description = substr($description, 0, 255); // Limiter la longueur
            }
            ...
            debug_log("Détail validé $index: pieces=$pieces, total=".$calc['total_value']." ".$calc['unit']);
        }
        ...
        if (empty($validated_details)) {
            ...
            echo json_encode(array(
                ...
                'debug' => $debug_mode ? array(
                    'original_count' => count($details_array),
                    'original_data' => $details_array
                ) : null
            ));
            exit;
        }

        debug_log("Détails validés: " . count($validated_details) . " sur " . count($details_array));

        $details_obj = new CommandeDetDetails($db);
        $result = $details_obj->saveDetailsForLine($commandedet_id, $validated_details, $user);
        
        if ($result < 0) {
            debug_log("ERREUR: Échec de la sauvegarde en base");
            http_response_code(500);
            echo json_encode(array(
                'success' => false, 
        ...
        debug_log("Sauvegarde réussie - " . count($validated_details) . " lignes");
        echo json_encode(array(
            'success' => true, 
